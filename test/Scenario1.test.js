...
contract('Scenario1', function (accounts) {
  ...
  it('Execute', async function () {
    ...
    await time.increase(2592000);
    await this.ethStaking.stake({ from: customer1, value: '24000000000000000000', gas: '5000000' });
    await time.increase(2592000);
  ...
  it('Is the staking times correct?', async function () {
    ...
    expect(stakingTimes).to.be.bignumber.equal(new BN('2'));
  });
  
  it('Is total withdrawal amount correct?', async function () {
    let totalWithdrawalAmount = await this.ethStaking.getTotalWithdrawalAmount(customer1);
    expect(totalWithdrawalAmount).to.be.bignumber.equal(new BN('9576000000000000000'));
  });
  ...
  it('Is total awarded eth2 amount correct?', async function () {
    ...
    expect(totalAwardedEth2Amount).to.be.bignumber.equal(new BN('137172120528569904436'));
  });
});