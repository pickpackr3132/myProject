...
contract('Scenario3', function (accounts) {
  ...
  it('Execute', async function () {
    await this.ethStaking.stake({ from: customer1, value: '36000000000000000000', gas: '5000000' });
    await time.increase(2592000);
    await this.ethStaking.stake({ from: customer1, value: '36000000000000000000', gas: '5000000' });
    await time.increase(2592000);
  ...
  it('Is total withdrawal amount correct?', async function () {
    ...
    expect(totalStakingAmount).to.be.bignumber.equal(new BN('23328000000000000000'));
  });
  ...
  it('Is total awarded eth2 amount correct?', async function () {
    ...
    expect(totalAwardedEth2Amount).to.be.bignumber.equal(new BN('308637271189282284981'));
  });
});